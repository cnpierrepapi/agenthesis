...
// Evaluate ONE strategy (a lever set, gated to certain edge kinds) against an
// edge. Pure. Returns { take, reason, ... }. `bankroll` and `openCount` are the
// agent-wide figures the strategy sizes / caps against.
export function evalStrategy(strat, edge, ctx, bankroll) {
  const L = strat.levers;

  if (!strat.edgeKinds.includes(edge.kind))
    return { take: false, reason: `strategy does not trade ${edge.kind}` };
  ...
  const stake =
    L.stakeMode === "kelly"
      ? kellyStake(bankroll, edge.edgeMeasure, odds, pEntry, L.kellyFraction)
      : bankroll * L.stakePct;
...
// The pure decision. An agent runs ONE OR MORE strategies: its always-on base
// tuning PLUS any attached research papers, each a calibrated strategy. We try
// them in priority order (papers first, then base) and take the FIRST that
// greenlights this edge — so the agent trades its base settings AND its papers,
// never double-staking one edge. No mutation, no clock.
export function decide(agent, edge, ctx) {
  ...
  let lastReason = "no strategy took this edge";
  for (const strat of strategies) {
    const d = evalStrategy(strat, edge, ctx, agent.bankroll);
    if (d.take) return d;
    lastReason = d.reason;
  }
  return { take: false, reason: lastReason };
}
